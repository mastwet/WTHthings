...
/*
unsigned int seq_bpm = 0;
unsigned int seq_seq_section = 1;

void change_playing_page(int knob_dire,int knob_number){
  switch(knob_dire){
    case KNOB_LEFT:
    //left
    if(knob_number == 4){
      //if(current_setting_note < 38) current_setting_note++;
      if(seq_bpm < 200) seq_bpm++;
    }
    else if(knob_number == 3){
      //seq 
      if(seq_seq_section < 8) seq_seq_section++;
      //if(current_setting_page == 1) current_setting_page = 2;
    }
    break;
    case KNOB_RIGHT:
    if(knob_number == 4){
      //if(current_setting_note > 0) current_setting_note--;
      if(seq_bpm > 0) seq_bpm--;
    }
    else if(knob_number == 3){
      //if(current_setting_page == 2) current_setting_page = 1;
      if(seq_seq_section > 0) seq_seq_section--;
    }
    break;
  }
}
...
void drawLogo(void) {
  ...
  for (int j = 0; j < 4; j++) {
    for (int i = 0; i < 4; i++) {
      if (cc_trigger_pad[j * 4 + i] == true) {
        u8g2.drawBox(12 + 10 * i + 2 * i, 4 + 10 * j + 2 * j, 10, 10);
      } 
      else {
        u8g2.drawFrame(12 + 10 * i + 2 * i, 4 + 10 * j + 2 * j, 10, 10);
      }
    }
  }
  
  //clo_draw row_draw
  int clo_draw = (sequenceIndex / 4);
  int row_draw = (sequenceIndex % 4);
  //Serial.printf("%d,%d,%d\n",clo_draw,row_draw,sequenceIndex);
  u8g2.drawFrame(15 + 10 * row_draw + 2 * row_draw, 7 + 10 * clo_draw + 2 * clo_draw, 4, 4);

  u8g2.setFont(u8g2_font_6x10_tf); // 可根据需要选择其他字体 
  u8g2.drawStr(0, 62, "BP:127 BA:0 P:0"); // 在底部显示一行字  

  for (int j = 0; j < 4; j++) {
    for (int i = 0; i < 4; i++) {
      u8g2.drawFrame(62 + 10 * i + 2 * i, 4 + 10 * j + 2 * j, 10, 10);
    }
  }
  ...
  // for (int j = 0; j < 2; j++) {
  //   for (int i = 0; i < 4; i++) {
  //     u8g2.drawFrame(20 + 10 * i + 12 * i, 20 + 10 * j + 8 * j, 18, 15);
  //   }
  // }

  // for (int j = 0; j < 4; j++) {
  //   for (int i = 0; i < 4; i++) {
  //     u8g2.drawFrame(25 + 10 * i + 10 * i, 7 + 10 * j + 4 * j, 17, 12);
  //   }
  // }
//当seq的模式为4时候，则只显示8个框,以此类推
//8个
...
void loop() {
  ...
  else if(h_message.type == 0x02 && h_message.number == 0x05 && h_message.message == 0x00 ){
    if(sequenze_play){
      sequenze_play = false;
      leds[sequenceIndex - 1] = CRGB::Black;
      sequenceIndex = 0;
      //vTaskSuspend(timerSemaphore);
      //timerStop(timer);
      //timerAlarmDisable(timer);
    }
    else{
      sequenze_play = true;
      //vTaskResume(timerSemaphore);
      //timerStart(timer);
      //timerAlarmEnable(timer);
    }
  }
