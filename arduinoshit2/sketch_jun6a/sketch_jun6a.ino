      ...
      if (packet_en == 1) {
        ...
        if (data == PACKET_END) {
          ...
          if (packet[1] == 0x01) {  // Button
            midimessage.type = 0;
            midimessage.note_pitch = keypad_map[packet[1]];
          ...
          // Knob
          else if (packet[1] == 0x02) {  // Knob
            if (packet[2] == 0x01) {     // Knob left
              midimessage.type = 1;
              if(knob_value[packet[3]] > 0){
                knob_value[packet[3]]++;
              }
            } 
            if (packet[2] == 0x01) {     // Knob left
              midimessage.type = 1;
              if(knob_value[packet[3]] < 127){
                knob_value[packet[3]]--;
              }
            } 

            midimessage.type = 1;
            midimessage.note_pitch = knob_value[packet[3]];
          }
          // Button
